...
def get_arguments():
    ...
    parser.add_argument('--checkpoint-dir',
                        type=str,
                        default=settings.TRAINCONF['s2']['checkpoints_dir'],
                        help='Checkpoints dir')
    parser.add_argument('--runs-dir',
                        type=str,
                        default=settings.TRAINCONF['s2']['runs_dir'],
                        help='Runs dir')
    parser.add_argument('--resume',
                        type=bool,
                        default=settings.TRAINCONF['s2']['resume'],
                        help='Default is False, change if want to resume')
    parser.add_argument('--load-best',
                        type=bool,
                        default=settings.TRAINCONF['s2']['load_best'],
                        help='Default is False, load checkpoint based best IOU')
    parser.add_argument('--load-epoch',
                        type=int,
                        default=settings.TRAINCONF['s2']['load_epoch'],
                        help='load checkpoint based epoch')
    parser.add_argument('--num-epochs',
                        type=int,
                        default=settings.TRAINCONF['s2']['num_epochs'],
                        help='Number of epochs for training')
    parser.add_argument('--train-batch',
                        type=int,
                        default=settings.TRAINCONF['s2']['tr_batch_sizes'],
                        help='Training batch size')
    parser.add_argument('--test-batch',
                        type=int,
                        default=settings.TRAINCONF['s2']['ts_batch_sizes'],
                        help='Testing batch size')
    parser.add_argument('--lr',
                        type=float,
                        default=settings.TRAINCONF['s2']['lr'],
                        help='Learning rate')
    parser.add_argument('--wd',
                        type=float,
                        default=settings.TRAINCONF['s2']['wd'],
                        help='Weights decay')
    parser.add_argument('--momentum',
                        type=float,
                        default=settings.TRAINCONF['s2']['momentum'],
                        help='Momentum')
    parser.add_argument('--snapshot',
                        type=int,
                        default=settings.TRAINCONF['s2']['snapshot'],
                        help='Store a model every snapshot epochs')
    parser.add_argument('--use-test',
                        action='store_true',
                        default=settings.TRAINCONF['s2']['use_test'],
                        help='See evolution of the test set when training?')
    parser.add_argument('--test-session',
                        type=int,
                        default=settings.TRAINCONF['s2']['test_session'],
...
def main():
    ...
    model_name = 'IMCNET'
    model_init = ModelBuild(arch=args.arch, frozen_layers=settings.TRAINCONF['s2']['frozen_layers'],
    ...
    model = model_init.get_model(model_path=settings.TRAINCONF['s2']['pre_trained'])
    params = [{'params': model.feature_extractor.parameters(), 'lr': args.lr * 0.01},
              ...
              {'params': model.segmentation.parameters(), 'lr': args.lr}]
    ...
    data_init = DatasetBuild(dataset_list=settings.TRAINCONF['s2']['dataset_list'],
                             dataset_conf=settings.DATASET_CONF)
    print('=====> 1.Loading training dataset <=====')
    trainloader = data_init.get_multi_dataset(dset_list=settings.TRAINCONF['s2']['dataset_list'], train=True)
    ...
    scheduler = optim.lr_scheduler.MultiStepLR(optimizer, [35, 75], gamma=0.5)
